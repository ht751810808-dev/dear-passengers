...
const pages = [
  { route: 'dear-passengers-switch', locale: 'en', keyword: 'Dear Passengers Switch', checkCharacters: false },
  { route: 'de/dear-passengers-switch', locale: 'de-DE', keyword: 'Dear Passengers Switch', checkCharacters: true },
];
...
for (const page of pages) {
  ...
  const initiallyVisibleBody = textWithoutMarkup(removeHiddenSubtrees(body));
  const indexableBody = textWithoutMarkup(body);
  ...
  const initiallyVisible = audit(initiallyVisibleBody);
  const indexableIncludingExpandedFaq = audit(indexableBody);
  const initiallyVisibleMainWordUnits = wordUnits(mainText).length;
  ...
  console.log(JSON.stringify({
    ...
    initiallyVisibleMainWordUnits,
    meaningfulParagraphCharacters: page.checkCharacters ? meaningfulParagraphCharacters : null,
